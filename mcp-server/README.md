...
- `palette(assignments)` — set different colors / Kelvin values on different groups in one round trip (key/fill/back in a single call)
- `strobe(rate, intensity?, groups?)` — strobe targeted fixtures at a given Hz rate (0–20Hz, or "off"). First-class abstraction over each fixture's dedicated strobe channel.
